...
const Services = () => {
  ...
  return (
    <section id="services" className="py-20 bg-gray-900 bg-grid relative">
      {/* 装饰元素 */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute -left-32 top-20 w-64 h-64 bg-purple-600/10 rounded-full filter blur-3xl"></div>
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <motion.div
              key={index}
              className="bg-gray-800/80 backdrop-blur-sm rounded-2xl border border-gray-700 hover:shadow-lg transition-shadow p-8"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
